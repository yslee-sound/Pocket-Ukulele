...
### Unresolved reference 'eq' 에러
- Supabase Kotlin SDK는 직접 `.eq()` 체이닝 불가
- 모든 데이터를 가져온 후 `.filter {}`로 필터링
...
**완료!** 이제 Supabase를 활용한 공지사항 시스템을 사용할 수 있습니다! 🎉
